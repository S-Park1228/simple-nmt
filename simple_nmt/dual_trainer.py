...
class DualSupervisedTrainingEngine(Engine):
    ...
    @staticmethod
    def _reorder(x, y, l): # l: the lengths of y (ex. [1, 3, 2]), where the order follows the packed sequence of x
    ...
    def _get_loss(x, y, x_hat, y_hat, crits, x_lm=None, y_lm=None, lagrange=1e-3):
        # |x| = (batch_size, n)
        # |y| = (batch_size, m)
        # |x_hat| = (batch_size, n, output_size0), generated by y to x model, composed of log probabilities
        # |y_hat| = (batch_size, m, output_size1), generated by x to y model, composed of log probabilities
        # |x_lm| = |x_hat|
        # |y_lm| = |y_hat|
    ...
    def train(engine, mini_batch):
        for language_model, model, optimizer in zip(engine.language_models,
                                                    engine.models,
                                                    engine.optimizers):
            language_model.eval() # We just use the pretrained LM via forward and its parameters need to be freezed.
            model.train()
        ...
        with autocast(not engine.config.off_autocast):
            # X2Y
            x, y = (mini_batch.src[0][:, 1:-1], mini_batch.src[1] - 2), mini_batch.tgt[0][:, :-1] # x: w/o <BOS> and <EOS>
                                                                                                  # Thus, x length <- x length - 2.
                                                                                                  # x length: mini_batch.src[1]
                                                                                                  # y is going to be inserted,
                                                                                                  # so that we remove <EOS> from y.
            x_hat_lm, y_hat_lm = None, None
            # |x| = (batch_size, n)
            # |y| = (batch_size, m)
            y_hat = engine.models[X2Y](x, y) # teacher forcing
            ...
            x, y, restore_indice = DualSupervisedTrainingEngine._reorder(
                mini_batch.src[0][:, :-1], # It needs to be with <BOS> and without <EOS> since it will be inserted into Decoder.
                mini_batch.tgt[0][:, 1:-1], # It needs to be w/d <BOS> and <EOS> since it will be inserted into Encoder.
                mini_batch.tgt[1] - 2,
            )
            # |x| = (batch_size, n)
            # |y| = (batch_size, m)
            x_hat = DualSupervisedTrainingEngine._restore_order(
                engine.models[Y2X](y, x), # teacher forcing
            ...
            x, y = mini_batch.src[0][:, 1:], mini_batch.tgt[0][:, 1:] # groundtruth for each Decoder
    ...
    def attach(
        train_engine,
        validation_engine,
        training_metric_names = ['x2y', 'y2x', 'reg', '|param|', '|g_param|'], # 'reg': dual loss
...
class DualSupervisedTrainer():
    ...
    # Note that some arguments are written as plural terms, which is due to dual learning.
    def train(
        self,
        models, language_models, # pretrained lm
        crits, optimizers,
